<script lang="ts">
		...
		try {
			const res = await fetch(`${API_V1}/leads`, {
				...
				body: JSON.stringify({
					...
					notes: form.notes,
					lang: langStore.value
				})
			});
			if (!res.ok) throw new Error(`lead submit failed: ${res.status}`);
			submitted = true;
		} catch {
			submitError = langStore.t('order.submit_error');
		} finally {
			sending = false;
		}
	}
	...
	$effect(() => {
		...
		if (!type || page.url.searchParams.get('from')) return;
		if (form.types.length === 0) form.types = [type];
	});
</script>
...
{#snippet formNav(stepNum: number)}
	<div class="form-nav">
		...
		{:else}
			<button type="submit" class="btn-submit" disabled={!canNext || sending}>
				{sending ? langStore.t('order.sending') : langStore.t('order.submit')}
				{@render arrow(14)}
			</button>
		{/if}
	</div>
	{#if stepNum === TOTAL_STEPS && submitError}
		...
		<div class="form-side">
			...
			{#if submitted}
				...
				<div class="success-state" aria-live="polite" role="status">
					...
					<a href={l('/portfolio')} class="btn-submit" style="display:inline-flex;margin-top:0;">
...
<style>
	...
	.submit-error { margin-top: 16px; font-size: 13px; color: var(--c-accent2); text-align: right; }
