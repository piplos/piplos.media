...
export const actions: Actions = {
	...
	save: async (event) => {
		...
		const stackRes = await fetchWithAuth(event, '/v1/stack');
		...
		const res = await fetchWithAuth(event, id ? `/v1/stack/${id}` : '/v1/stack', {
			method: id ? 'PUT' : 'POST',
	...
	delete: async (event) => {
		...
		const res = await fetchWithAuth(event, `/v1/stack/${id}`, { method: 'DELETE' });
	...
	reorder: async (event) => {
		...
		const res = await fetchWithAuth(event, '/v1/stack/reorder', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
