<script lang="ts">
	...
	let groupsOverride = $state<ProjectGroup[] | null>(null);
	...
	const baseGroups = $derived(buildGroups(data.services, data.projects));
	...
	function moveItem(fromId: string, toGroupId: string, beforeItemId: string | null): ProjectGroup[] | null {
	...
	function findItemIn(source: ProjectGroup[], itemId: string): { groupIdx: number; itemIdx: number } | null {
	...
	async function persistLayout(nextGroups: ProjectGroup[]) {
	...
	const visibleGroups = $derived(
		data.category === ''
			? groups
			: groups.filter((g) => g.id === data.category)
	);

	const hasItems = $derived(visibleGroups.some((group) => group.items.length > 0));
	const showGroupHeaders = $derived(data.category === '');
</script>
					...
					<table class="chart-table">
						...
						<tbody ondragover={(e) => e.preventDefault()}>
							{#each visibleGroups as group (group.id)}
								{#if showGroupHeaders && (group.items.length > 0 || group.id !== ORPHAN_GROUP_ID)}
									<tr
