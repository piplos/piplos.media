<script lang="ts">
	...
	$effect(() => {
		groups = buildGroups(data.services, data.stack);
	});
	...
	async function persistLayout() {
		...
		if (orphanGroup?.items.length) {
			toast.error('Переместите технологии без группы в одну из услуг');
			return;
		}
...
<svelte:head>
	<title>Списки: Стек — Piplos Admin</title>
</svelte:head>
